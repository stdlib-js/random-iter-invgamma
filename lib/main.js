...
function iterator( alpha, beta, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0Pq7P', alpha ) );
	}
	if ( !isPositiveNumber( beta ) ) {
		throw new TypeError( format( '0Pq7T', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pq2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pq35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
